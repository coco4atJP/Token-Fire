...
export class ControlCenter {
  ...
  constructor(
    ...
    this.root.innerHTML = `
      <header class="control-center__header">
        <strong>COMPANY LEDGER</strong>
        <button type="button" data-action="close" aria-label="閉じる">×</button>
      </header>
      <nav class="control-center__tabs" aria-label="記録棚のページ">
        <button type="button" data-tab="ledger">記録</button>
        <button type="button" data-tab="projects">事業所</button>
        <button type="button" data-tab="replays">動作</button>
        <button type="button" data-tab="events">できごと</button>
    ...
    this.body = this.requireElement(".control-center__body");
  ...
  private render(force: boolean): void {
    ...
    for (const button of this.root.querySelectorAll<HTMLButtonElement>("[data-tab]")) {
      button.setAttribute("aria-pressed", String(button.dataset.tab === this.activeTab));
    }
    if (!force && this.body.matches(":focus-within")) return;
    const renderers: Record<string, () => string> = {
      ledger: () => this.renderLedger(),
      projects: () => this.renderProjects(),
      replays: () => this.renderReplays(),
  ...
  private renderLedger(): string {
    ...
    return `
      <div class="ledger-metrics">
        ${metric("焼却Token", metrics.totalTokensBurned.toLocaleString())}
        ${metric("ふわっとした多さ", metrics.energyLabel)}
        ${metric("設備", `${metrics.growthLevel + 1}/24`)}
        ${metric("成果ゼロ", metrics.wastedTokens.toLocaleString())}
      </div>
      <div class="ledger-actions">
        <button type="button" data-action="report">環境債務報告書</button>
        <button type="button" data-action="export-database">全事業所データ</button>
      </div>
      <h3>最近こういうこともありました</h3>
      <div class="memory-list">${recent.length ? recent.map((moment) => `
        <article class="memory-item" data-importance="${moment.importance}">
          <strong>${escapeHtml(moment.title)}</strong>
          <p>${escapeHtml(moment.line)}</p>
          <time>${new Date(moment.at).toLocaleString("ja-JP")}</time>
        </article>`).join("") : "<p class=empty>まだ記録はありません。</p>"}
      </div>`;
  }

  private renderProjects(): string {
    const projects = this.persistence.listProjects();
    return `<p class="control-note">Codexの作業ディレクトリごとに、別の森と工場を自動で使います。</p>
      <div class="project-list">${projects.map((project) => `
        <article class="project-item ${project.key === this.world.projectKey ? "is-current" : ""}">
          <strong>${escapeHtml(project.label)}</strong>
          <span>${project.totalTokens.toLocaleString()} TOK · 設備 ${project.growthLevel + 1}/24</span>
          <small>${project.historyCount}件の記録 · ${project.replayCount}本の動作データ</small>
        </article>`).join("") || "<p class=empty>事業所はまだありません。</p>"}</div>`;
  }

  private renderReplays(): string {
    return `<p class="control-note">動画そのものは保存していません。軽量な動作データから、必要な時だけWebMを生成します。</p>
      <div class="replay-list">${this.world.replays.map((replay) => `
        <article class="replay-item">
          <strong>${escapeHtml(replay.title)}</strong>
          <span>${replay.totalTokens.toLocaleString()} TOK · ${replay.frames.length} frames · ${replay.wasted ? "未完了" : "完了"}</span>
          <div><button type="button" data-action="replay-video" data-replay="${escapeHtml(replay.id)}">動画化</button><button type="button" data-action="replay-data" data-replay="${escapeHtml(replay.id)}">動作データ</button></div>
        </article>`).join("") || "<p class=empty>完了したタスクの動作データがここへ静かに残ります。</p>"}</div>`;
  }
  ...
  private async handleClick(event: Event): Promise<void> {
    ...
    if (target.dataset.action === "replay-video" || target.dataset.action === "replay-data") {
      ...
      if (!replay) return;
      if (target.dataset.action === "replay-video") await exportReplayVideo(replay);
      else exportReplayData(replay);
    }
  ...
  private async handleChange(event: Event): Promise<void> {
    ...
    if (input.dataset.pack) {
      ...
      if ((input as HTMLInputElement).checked) current.add(input.dataset.pack);
      else current.delete(input.dataset.pack);
  ...
  private async importPack(event: Event): Promise<void> {
    ...
    if (!file) return;
    try {
      this.eventPacks.import(JSON.parse(await file.text()));
      this.render(true);
    } catch (error) {
      window.alert(error instanceof Error ? error.message : "イベントパックを読み込めませんでした");
    } finally {
      input.value = "";
    }
  }
